...
const AttestationStatusCard = memo(function AttestationStatusCard() {
  ...
  const badgeColor = (placement?: number) => {
    if (placement === 1) return 'bg-[rgba(221,180,105,0.2)] text-[var(--color-gold)] border-[rgba(221,180,105,0.4)]';
    if (placement === 2) return 'bg-[rgba(192,192,192,0.15)] text-[#c0c0c0] border-[rgba(192,192,192,0.3)]';
    if (placement === 3) return 'bg-[rgba(205,127,50,0.15)] text-[#cd7f32] border-[rgba(205,127,50,0.3)]';
          ...
          attestationReadiness.slice(0, 5).map((attestation) => (
            ...
            >
              <div className="flex justify-between items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[9px] tracking-[0.08em] uppercase border ${badgeColor(attestation.placement)}`}>
              ...
              <div className="mt-1.5 grid grid-cols-4 gap-2 text-[9px] text-[var(--color-text-muted)]">
                ...
                {attestation.trustDelta !== undefined && (
                  <div>
                    ...
                    <span className={`font-mono ${trendColor(attestation.trustDelta)}`}>
                      {attestation.trustDelta > 0 ? '+' : ''}{attestation.trustDelta.toFixed(2)}
                    </span>
                  </div>
                )}
                {attestation.cooperationRate !== undefined && (
                  <div>
                    ...
                    <span className="font-mono">{(attestation.cooperationRate * 100).toFixed(0)}%</span>
                  </div>
                )}
