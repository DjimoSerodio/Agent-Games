            ...
            >
                ...
                <div className="grid gap-2 min-w-0">
                  <div className="flex items-baseline gap-4">
                    <div className="flex items-baseline gap-2">
                      <div className="font-mono text-[10px] tracking-[0.14em] uppercase text-[var(--color-text-soft)]">VP</div>
                      <div className="font-serif text-xl leading-none text-[var(--color-text)]">{agent.vp || 0}</div>
                    </div>
                    <div className="flex items-baseline gap-2">
                      <div className="font-mono text-[10px] tracking-[0.14em] uppercase text-[var(--color-text-soft)]">INF</div>
                      <div className="font-serif text-xl leading-none text-[var(--color-text)]">{agent.influence || 0}</div>
                    </div>
                    <div className="flex items-baseline gap-2">
                      <div className="font-mono text-[10px] tracking-[0.14em] uppercase text-[var(--color-text-soft)]">TRUST</div>
                      <div className="font-serif text-xl leading-none text-[var(--color-text)]">{Number(agent.trust || 0).toFixed(2)}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3.5 mt-4">
                {[
                  ...
                  <div key={res.key} className="rounded-xl p-3 border border-[rgba(233,220,190,0.1)] bg-[rgba(8,12,8,0.26)] text-center flex flex-col items-center gap-1.5">
              ...
              <div className="flex flex-wrap gap-3 mt-4">
                {[
                  ...
                  const count = agent.structures?.[struct.key as keyof NonNullable<typeof agent.structures>] || 0;
                  if (count === 0) return null;
                  return (
                      <div key={struct.key} className="px-3 py-1.5 rounded-full border border-[rgba(233,220,190,0.12)] bg-[rgba(33,43,33,0.5)] text-[10px] text-[var(--color-text-muted)]">
                      {count} {struct.label}
                    </div>
                  );
                })}
