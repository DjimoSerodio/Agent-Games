...
function tileBorder(row: number, col: number): string {
  ...
  if (coopNorm > 0.6) {
    ...
    return toRgba(mixColor(TILE_GREEN, TILE_GREEN_BRIGHT, t * 0.75 + relNorm * 0.2), 0.26 + relNorm * 0.14 + t * 0.08);
  }

  if (coopNorm < 0.4) {
    const t = (0.4 - coopNorm) / 0.4;
    return toRgba(mixColor(TILE_RED, TILE_RED_BRIGHT, t * 0.8 + (1 - relNorm) * 0.12), 0.24 + (1 - relNorm) * 0.08 + t * 0.1);
  }
...
const pendingRef = useRef<{ agentId: string; round: number; row: number; col: number } | null>(null);
const lastKeyRef = useRef<Record<string, string>>({});
  ...
  const flush = () => {
    const { agentId: id, round: rnd, row: r, col: c } = pendingRef.current ?? { agentId: null, round: 0, row: 0, col: 0 };
    if (!id) return;
    const key = `${rnd}:${r}:${c}`;
      ...
      <div className="grid grid-cols-[auto_1fr] gap-3 items-stretch">
        ...
        >
          {Array.from({ length: SPECTRUM_SIZE * SPECTRUM_SIZE }).map((_, idx) => {
            ...
            const isTrail =
              trailIdx !== undefined && !(isCurrent && trailIdx === trail.length - 1);
            const bg = tileTone(row, col);
            const border = tileBorder(row, col);
            const trailOpacity =
              isTrail && trail.length > 1
                ? 0.25 + (trailIdx! / Math.max(1, trail.length - 1)) * 0.55
                : 0;
            return (
              <div
                key={`${row}-${col}`}
                className="relative aspect-square rounded-md border flex items-center justify-center transition-colors cursor-pointer"
                style={{
                  backgroundColor: bg,
      ...
      <div className="grid grid-cols-[auto_1fr] gap-3 items-start">
        ...
        >
          {RELIABILITY_LABELS.map((label, i) => (
            <div
              key={label}
              className="relative h-[42px] min-w-0"
            >
              <span
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="font-mono text-[10px] tracking-[0.18em] uppercase text-[var(--color-text-soft)]">
          Row {inspectedRow + 1} · Col {inspectedCol + 1}{!inspectingCurrent ? ' · Inspecting tile' : ''}
        </div>
      </div>
      ...
      >
        ...
        {!inspectingCurrent && (
          <div className="mt-3 rounded-[14px] border border-[rgba(233,220,190,0.12)] bg-[rgba(11,23,34,0.42)] px-4 py-3 text-[12px] leading-[1.5] text-[var(--color-text-muted)]">
            ...
            <div className="mt-1">
              The detail lists below stay tied to this agent&apos;s live evidence, while the title above previews the clicked spectrum tile.
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <ul className="mt-2 space-y-1 text-[12px] text-[var(--color-text-muted)]">
              {currentPosition.reasons.cooperation.length > 0 ? (
                currentPosition.reasons.cooperation.map((line, i) => (
          ...
          <div>
            ...
            <ul className="mt-2 space-y-1 text-[12px] text-[var(--color-text-muted)]">
              {currentPosition.reasons.reliability.length > 0 ? (
                currentPosition.reasons.reliability.map((line, i) => (
                  <li key={i} className="leading-snug">
                    · {line}
                  </li>
                ))
      ...
      {(recentCommitmentSummary.length > 0 || recentTags.length > 0) && (
        <div className="rounded-[14px] border border-[rgba(233,220,190,0.12)] p-5 bg-[rgba(11,23,34,0.5)]">
          ...
          <ul className="mt-2 space-y-1.5 text-[13px] text-[var(--color-text-muted)] leading-[1.55]">
            {recentCommitmentSummary.map((line, i) => (
              <li key={`c-${i}`}>· {line}</li>
            ))}
            {recentTags.map((line, i) => (
              <li key={`t-${i}`}>· {line}</li>
            ))}
          </ul>
        </div>
