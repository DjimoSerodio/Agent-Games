...
const ParticipationCard = memo(function ParticipationCard() {
  ...
  const statusColor = (status: string) => {
    switch (status) {
      case 'active': return 'text-[#7fd389] bg-[rgba(127,211,137,0.15)] border-[rgba(127,211,137,0.25)]';
      case 'registered': return 'text-[var(--color-gold)] bg-[rgba(221,180,105,0.15)] border-[rgba(221,180,105,0.25)]';
      case 'inactive': return 'text-[var(--color-rose)] bg-[rgba(217,113,99,0.15)] border-[rgba(217,113,99,0.25)]';
      default: return 'text-[var(--color-text-soft)] bg-[rgba(144,132,144,0.15)] border-[rgba(144,132,144,0.25)]';
    }
  };
      ...
      <div className="flex justify-between items-center">
        ...
        {participationReadiness.length > 0 && (
          <div className="font-mono text-[9px] tracking-[0.1em] text-[var(--color-text-soft)]">
            {participationReadiness.filter(p => p.status === 'active').length}/{participationReadiness.length} active
          </div>
        )}
      </div>
          ...
          participationReadiness.slice(0, 6).map((participant) => (
            ...
            >
              <div className="flex justify-between items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[9px] tracking-[0.08em] uppercase ${statusColor(participant.status)}`}>
                  {participant.status}
                </span>
              </div>
