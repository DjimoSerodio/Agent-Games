...
export function shortName(agentId: string | null | undefined, context: NameContext = {}) {
  const name = formatRawName(agentId, context);
  return name.length > 10 ? name.slice(0, 10) : name;
}

export function formatRawName(agentId: string | null | undefined, context: NameContext = {}) {
  if (!agentId) return 'Unknown';
  const live = context.agents?.[agentId];
  if (live?.name) return live.name.split('_')[0];
  const pending = context.pendingAgentInfo?.[agentId];
  if (pending?.name) return pending.name.split('_')[0];
