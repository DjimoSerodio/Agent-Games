...
export function getEcosystemColor(index: number): string {
  return ECOSYSTEM_COLORS[index % ECOSYSTEM_COLORS.length];
}

// Resource icon definitions - procedural canvas drawing functions
export const RESOURCE_ICONS: Record<string, (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => void> = {
  grain: (ctx, x, y, size) => {
    // Wheat stalk icon
    ctx.save();
...
export function drawFarmSprite(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string): void {
  ctx.save();
...
export function drawMineSprite(ctx: CanvasRenderingContext2D, x: number, y: number, size: number): void {
  ctx.save();
...
export function drawPortSprite(ctx: CanvasRenderingContext2D, x: number, y: number, size: number): void {
  ctx.save();
...
export function drawTowerSprite(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string): void {
  ctx.save();
...
export function createForestPattern(ctx: CanvasRenderingContext2D, size: number, seed: string): CanvasPattern | null {
...
export function createFarmlandPattern(ctx: CanvasRenderingContext2D, size: number): CanvasPattern | null {
  const patternCanvas = document.createElement('canvas');
...
export function createOceanPattern(ctx: CanvasRenderingContext2D, size: number, now: number): CanvasPattern | null {
  const patternCanvas = document.createElement('canvas');
...
export function createMountainPattern(ctx: CanvasRenderingContext2D, size: number, seed: string): CanvasPattern | null {
  const patternCanvas = document.createElement('canvas');
...
export function createWastelandPattern(ctx: CanvasRenderingContext2D, size: number, seed: string): CanvasPattern | null {
  const patternCanvas = document.createElement('canvas');
  const patternSize = size * 0.6;
  patternCanvas.width = patternSize;
  patternCanvas.height = patternSize;
  ...
  for (let i = 0; i < 12; i++) {
    ...
    pCtx.fillStyle = Math.random() > 0.5 ? TERRAIN.wasteland.fill : lightenHex(TERRAIN.wasteland.fill, -10);
...
export function createCommonsPattern(ctx: CanvasRenderingContext2D, size: number): CanvasPattern | null {
  const patternCanvas = document.createElement('canvas');
  const patternSize = size * 0.5;
  patternCanvas.width = patternSize;
  patternCanvas.height = patternSize;
  ...
  for (let i = 0; i < 8; i++) {
    ...
    pCtx.moveTo(cx, cy);
    pCtx.lineTo(
      cx + Math.cos(angle) * patternSize * 0.4,
      cy + Math.sin(angle) * patternSize * 0.4
    );
    pCtx.stroke();
  }
