...
function tagPolarity(tag: VisibleBehaviorTag): -1 | 0 | 1 {
  ...
  if (kind && kind in BEHAVIOR_KIND_POLARITY) {
    return BEHAVIOR_KIND_POLARITY[kind];
  }
...
export function computeTypology(ctx: TypologyContext): TypologyResult {
  ...
  if (c.total > 0) {
    reasons.cooperation.push(
      `${c.fulfilled} fulfilled / ${c.breached} breached commitments`,
    );
  }
  if (tags.positive || tags.negative) {
    reasons.cooperation.push(`${tags.positive} pro-social · ${tags.negative} hostile behavior tags`);
  }
  ...
  return {
    row,
    col,
    cooperation: COOPERATION_LABELS[row],
    reliability: RELIABILITY_LABELS[col],
    cooperationScore,
    reliabilityScore,
    reasons,
    stats: {
...
export function describeTile(row: number, col: number): string {
  ...
  if (row >= 5 && col >= 5) return `${combined} — keeps promises in plain view; a keystone peers lean on.`;
  if (row >= 5 && col <= 1) return `${combined} — warm intent, wobbly follow-through; allies hedge.`;
  if (row <= 1 && col >= 5) return `${combined} — reliably adversarial; you can set your watch by the betrayal.`;
  if (row <= 1 && col <= 1) return `${combined} — volatile and hostile; peers brace, not trust.`;
  if (row === 3 && col === 3) return `${combined} — the table reads this agent as an unknown, watching for a tell.`;
