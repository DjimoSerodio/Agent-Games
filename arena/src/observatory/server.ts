...
import { createTragedyWorldMap } from "../games/nexus/world-map.js";
...
export class ObservatoryServer {
  ...
  private worldTemplate = createTragedyWorldMap();
  ...
  private async runSimulation(agentType: SimulationAgentType = "llm", provider?: LLMProvider): Promise<void> {
    ...
    const { TragedyEngine } = await import("../games/nexus/tragedy-engine.js");
    ...
    const engine = new TragedyEngine(config, this.eventBus, this.trustGraph);
    ...
    if (agentType === "llm") {
      ...
      const strategies = [
        ...
        { name: "Bob_Builder", strategy: "builder" as const },
        { name: "Charlie_Opportunist", strategy: "opportunist" as const },
